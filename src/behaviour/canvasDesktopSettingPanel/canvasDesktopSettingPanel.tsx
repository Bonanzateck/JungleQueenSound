...
import { desktopSettingPanelActions } from "@bonanzainteractive/slote_core";
import { buttonActions } from "@bonanzainteractive/slote_core";
import { keyboardListenerActions } from "@bonanzainteractive/slote_core";
...
import { isFullScreen,toggleFullScreen } from "@bonanzainteractive/core";
    ...
    componentDidMount() {
        ...
        if (this.props.showSettingPanelUI && !isMobile) {
            ...
            if (!this.props.EnableIntroScreenOption) {
                this.UIManagerRef("introScreenRadioButtonOn").visible = false;
                this.UIManagerRef("introScreenRadioButtonOff").visible = false;
                this.UIManagerRef("introScreenRadioButtonDisable").visible = true;
            }

            if (!this.props.EnableTurboModeOption) {
                this.UIManagerRef("turboSpinRadioButtonOn").visible = false;
                this.UIManagerRef("turboSpinRadioButtonOff").visible = false;
                this.UIManagerRef("turboSpinRadioButtonDisable").visible = true;
            }
            if (!this.props.soundOnOff) {
                this.UIManagerRef("musicRadioButtonOn").visible = false;
                this.UIManagerRef("musicRadioButtonOff").visible = false;
                this.UIManagerRef("musicRadioButtonDisable").visible = true;
            } else {
                this.UIManagerRef("musicRadioButtonDisable").visible = false;
            }
            this.props.disableQuickSpin ? this.UIManagerRef("turboSpinRadioButtonDisable").visible = true : this.UIManagerRef("turboSpinRadioButtonDisable").visible = false;
            !this.props.showFullScreenButton ? this.UIManagerRef("fullScreenRadioButtonDisable").visible = true : this.UIManagerRef("fullScreenRadioButtonDisable").visible = false;
        }
    }
    ...
    setImageposition() {
        if (!isMobile)
            switch (this.props.languageCode) {
                case 'bg': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 758);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 973);
                    break;
                case 'cs':
                case 'ro': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 824);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 975);
                    break;
                case 'da':
                case 'nl': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 794);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 977);
                    break;
                case 'de':
                case 'gr': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 766);
                    break;
                case 'en': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 815);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 974);
                    break;
                case 'es': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 716);
                    break;
                case 'fi': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 804);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 978);
                    break;
                case 'fr': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 801);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 977);
                    break;
                case 'hr':
                case 'sr':
                case 'tr': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 823);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 977);
                    break;
                case 'hu':
                case 'nb': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 788);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 977);
                    break;
                case 'it': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 756);
                    break;
                case 'pl': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 814);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 978);
                    break;
                case 'pt': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 748);
                    break;
                case 'ru': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 800);
                    break;
                case 'sv': this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 783);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 978);
                    break;
                default:
                    this.UIManagerRef("settingImage") && (this.UIManagerRef("settingImage").x = 775);
                    this.UIManagerRef("settingHeading2") && (this.UIManagerRef("settingHeading2").x = 974);
    ...
    handleEvent = (e: any) => {
        ...
        switch (e.target.name) {
            ...
            case "spaceBarToSpinRadioButtonOn":
                ...
                this.radioController("spaceBarToSpinRadioButtonOn", "spaceBarToSpinRadioButtonOff");
                localStorage.setItem("playerId-SpaceBar-" + this.useQuery().get("token"), "false");
                return;
            case "spaceBarToSpinRadioButtonOff":
                this.props.spaceBarSpin(true);
                this.radioController("spaceBarToSpinRadioButtonOff", "spaceBarToSpinRadioButtonOn");
                localStorage.setItem("playerId-SpaceBar-" + this.useQuery().get("token"), "true");
                return;
